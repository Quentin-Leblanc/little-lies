...
const GroundFog = React.memo(function GroundFog({ isDay = true }) {
  ...
  const farOpacity = isDay ? 0.5 : 0.6;

  return (
    <>
      {/* One ground-hugging mist slab, not three. The stack at y=0.15 /
          0.45 / 0.85 read as a single opaque sheet from every camera
          angle above 40° — which is all of them — while costing three
          full-screen passes of 5-octave fBm. The remaining slab sits
          low enough to skim the ground and leave the characters clear. */}
      <MistSlab y={0.15} scale={1.0} speed={0.025} density={dense * 0.8} color={color} seed={0.1} />
